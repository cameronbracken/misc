...
cacheSweaveSetup <- function(file, syntax,
                             ...
                             eps=TRUE, pgf=FALSE, pgfpdf=FALSE, cache = FALSE,
                             external=FALSE) {
...
writeChunkMetadata <- function(object, chunk, options) {
        ...
        if(options$fig && options$eval) {
                figname <- if(options$pdf | options$pgfpdf)
...
cacheSweaveRuncode <- function(object, chunk, options) {
        ...
        if(options$fig && options$eval){
                ...
                if(options$pgf){
                        ...
                        grDevices::dev.off()
                        if(inherits(err, "try-error")) stop(err)
                                                err <- try(system(paste('eps2pgf',
                                                    paste(chunkprefix, "eps",sep="."),"-m directcopy")))
                        if(inherits(err, "try-error")) stop(err)
                                                unlink(paste(chunkprefix,"eps",sep="."))
                }
                if(options$pgfpdf){
                        grDevices::postscript(file=paste(chunkprefix, "eps", sep="."),
                                              width=options$width, height=options$height,
                        ...
                        if(inherits(err, "try-error")) stop(err)
                                                ...
                                                figTemp <- system.file(package='pgfWeave')
                                                figTemp <- paste(figTemp,'/misc/figTemp.tex',sep='')
                                                figFile <- file(figTemp,open='r')
                                                figTex <- scan(figFile,what=character(),sep='\n')
                                                begin <- grep('%PGF',figTex)

                                                figName <- strsplit(chunkprefix,'/')[[1]]
                                                figName <- figName[length(figName)]
                                                figPath <- 'figs'
        
                                                figTex <- c(figTex[1:begin],
                            paste("\\input{", paste(figName,'pgf',sep='.'),
                                                        "}\n", sep=""),
                                                    figTex[begin+1:length(figTex)])

                                                write(file=paste(chunkprefix,'tex',sep='.'),figTex) 

                                                err <- try(system(
                                                    paste('pdflatex','-output-directory',figPath,
                                                        paste(chunkprefix,'tex',sep='.'))))
                        if(inherits(err, "try-error")) stop(err)

                }
                if(options$external){
                            #will not work if pgf=TRUE has never been specified
                        if(chunkChanged | options$pgf){
                                grDevices::postscript(file=paste(chunkprefix, "eps", sep="."),
                                              width=options$width, height=options$height,
                                              paper="special", horizontal=FALSE)
                                ...
                                cat('pdflatex --jobname=',chunkprefix,' %MAIN\n',
                ...
                if(options$include && options$external) {
                        cat("\\beginpgfgraphicnamed{",chunkprefix,"}\n",
                            "\\input{", paste(chunkprefix,'pgf',sep='.'), "}\n",
                            "\\endpgfgraphicnamed\n",
                            sep="",file=object$output, append=TRUE)
                        linesout[thisline + 1] <- srcline
                        thisline <- thisline + 3
